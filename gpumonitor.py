...
while (True):
    ...
    for info in pidProcessMemorys:
        ...
        if(len(infodetail) == 3):
            ...
            if pid.isdigit() and pid not in saveKnownPIDS:
                ...
                # Create string
                messageString = "### New GPU Job\n User **@" + username + "** has created a job named **" + \
                    processName + "** on GPU with pid **" + pid + \
                    "** consuming **" + processMemory + "** memory."
                print(messageString)

                messageData = {'text': messageString,
                               'icon_url': nvidiaLogoLink}
                print(messageData)
                response = requests.post(mattermostIncomingWebhook, data=json.dumps(
                    messageData), headers={'Content-Type': 'application/json'})
    ...
    if showStatusUpdate:
        ...
        response = requests.post(mattermostIncomingWebhook, data=json.dumps(
            statusData), headers={'Content-Type': 'application/json'})
        if response.status_code != 200:
            print('Request error ', response.status_code,
                  ' the response is:\n', response.text)
    ...
    for pid in finishedPIDS:
        ...
        if username != None and processName != None:
            ...
            messageString = "### Finished GPU Job\n User **@" + username + \
                "** your job named **" + processName + \
                "** with pid **" + pid + "** has finished."
            print(messageString)
